...
def calcQDsc(parrDsc, parrLbl, pkmeans):
    ...
    numC     = kmeans.n_clusters
    tmpBins  = list(range(numC))
    lstIdx   = np.sort(np.unique(parrLbl[:, 0]))
    dscq = []
    lblq = []
    print('[*] calculate Quantized descriptors...')
...
if __name__ == '__main__':
    ...
    if not isUseCUDA:
        ...
        pathKCls = '{}-kcls-n{}-k{}-cuda.pkl'.format(pathIdxTrn, paramN, paramK)
    ...
    # kclsCuda = CudaKMeans(n_clusters=256)
    # kclsCuda.fit(arrDscRnd)
    # tmp = kclsCuda.predict(arrDscRnd)
    ...
    if os.path.isfile(pathKCls):
        ...
        print ('[**] Build KMeans model, (on GPU=={})'.format(pathKCls, isUseCUDA))
        if not isUseCUDA:
            kmeans = skcl.KMeans(n_clusters=paramK, n_jobs=4).fit(arrDscRnd)
        else:
            kmeans = CudaKMeans(n_clusters=paramK)
            kmeans.fit(arrDscRnd)
